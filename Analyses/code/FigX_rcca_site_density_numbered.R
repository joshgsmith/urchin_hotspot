...
g1 <- ggplot() +
  ...
  geom_text(urch_den_spatial %>% filter(species == "red urchin",
                                                       region == "Humboldt") %>%
                             mutate(Long = Long-0.12),
                           mapping=aes(x=Long, y=Lat, label=site_number), 
                           inherit.aes = F, size=2, color="black")+
...
g_legend1 <- ggplot() +
  ...
  scale_color_manual(name="Species", values=c( "purple", "darkred")) +
  scale_shape_manual(name="Monitoring program", values=c(16, 17)) +
  scale_size_binned(breaks = c(0.56,6.34,16.63),
                   # labels = function(x) as.character(round(x,0)),
                    name = "Sea urchin density \n(no. per m²)")+
  guides(color = guide_legend(order = 1), shape = guide_legend(order = 2)) +
...
g_legend4 <- ggplot() +
  ...
  scale_color_manual(name="Species", values=c( "purple", "darkred")) +
  scale_shape_manual(name="Monitoring program", values=c(16, 17)) +
  scale_size_binned(breaks = c(0.23,0.81,1.52),
                    #labels = function(x) as.character(round(x,0)),
                    name = "Red urchin density \n(no. per m²)")+
  guides(color = guide_legend(order = 1), shape = guide_legend(order = 2)) +
...
legend4 <- get_only_legend(g_legend4)


g <- gridExtra::grid.arrange(combined_plot, legend1, legend2, legend4,
                             ncol=1,
                             heights = c(10,0.5,1,1))
