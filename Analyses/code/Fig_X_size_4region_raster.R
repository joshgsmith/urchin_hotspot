...
ggplot() +
  ...
  geom_sf(data = size_pur_T, aes(col = u_diff))+
...
g_legend1 <- ggplot(usa) + 
  geom_sf(data = usa)+
  #add counties
  geom_sf(data = counties_T)+
  #add survey sites
  geom_sf(data = size_pur_T, size=1.5)+
  geom_spatraster(data=idwr) +  
  scale_fill_whitebox_c(
    palette = "muted",
    labels = scales::label_number(#suffix = "Density",
...
# Export figure
ggsave(g, filename=file.path(figdir, "purp_size_4region_raster.png"), 
